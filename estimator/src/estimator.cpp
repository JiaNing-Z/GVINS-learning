...
void Estimator::inputEphem(EphemBasePtr ephem_ptr)
{
    ...
    {
        sat2ephem[ephem_ptr->sat].emplace_back(ephem_ptr);//将整个指针存入到map中，第一位对应卫星颗数，第二位对应着数据信息
        sat2time_index[ephem_ptr->sat].emplace(toe, sat2ephem.at(ephem_ptr->sat).size()-1);//第一位是卫星数目，第二位是时间，第三位是EphemBasePtr指向数据的大小
    }
}
...
void Estimator::processGNSS(const std::vector<ObsPtr> &gnss_meas)
{
    ...
    for (auto obs : gnss_meas)//遍历所有的GNSS观测
    {
        ...
        //如果观测到的卫星数目为0，continue
        if (sat2ephem.count(obs->sat) == 0)
            continue;
        //根据频率选择，观测值和星历信息，有着不同的发射频率，这里选择L1频率
        if (obs->freqs.empty())    continue;       // no valid signal measurement
        int freq_idx = -1;
        L1_freq(obs, &freq_idx);
        if (freq_idx < 0)   continue;              // no L1 observation
        // 4、根据GNSS时间（秒）来判断星历是否可用；GPS系统星历每2小时更新一次, 正常情况下, 在偶数整点更新。
        double obs_time = time2sec(obs->time);//GNSS观测到的时间
        std::map<double, size_t> time2index = sat2time_index.at(obs->sat);//对应几颗卫星观测到的map
        double ephem_time = EPH_VALID_SECONDS;//系统并不是只可以运行2h，因为你的星历信息是不断累加的，//!但是这样会不会造成数据冗余，导致后面的判断越来越缓慢 
        size_t ephem_index = -1;
        for (auto ti : time2index)//遍历这个卫星数目下的所有的time2index
        {
